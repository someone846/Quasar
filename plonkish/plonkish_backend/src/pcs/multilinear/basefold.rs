...
pub struct BasefoldCommitment<F: PrimeField, H: Hash> {
    ...
    // Batch Merkle commitment used by `batch_open`.
    //
    // When several BaseFold codewords are committed together, we also build a
    // vector-leaf Merkle tree whose i-th leaf hashes all sibling-pair openings
    // at position i across the batch:
    //   H(C_0[2i], C_0[2i+1], ..., C_{m-1}[2i], C_{m-1}[2i+1]).
    //
    // This allows the batch opening to authenticate all committed codewords at
    // a queried position with one Merkle path, rather than one Merkle path per
    // codeword. The full tree is stored only on the first prover commitment in
    // the batch; every verifier commitment stores the public batch root.
    pub batch_codeword_tree: Vec<Vec<Output<H>>>,
    pub batch_root: Option<Output<H>>,
...
impl<F: PrimeField, H: Hash> PartialEq for BasefoldCommitment<F, H> {
    fn eq(&self, other: &Self) -> bool {
        self.codeword.poly.eq(&other.codeword.poly)
            ...
            && self.batch_codeword_tree.eq(&other.batch_codeword_tree)
            && self.batch_root.eq(&other.batch_root)
...
fn commit_without_individual_tree<F, H>(
    pp: &BasefoldProverParams<F>,
    poly: &MultilinearPolynomial<F>,
) -> Result<BasefoldCommitment<F, H>, Error>
where
    ...
    H: Hash,
{
    ...
    let codeword = if pp.rs_basecode {
    ...
    Ok(BasefoldCommitment {
        codeword,
        // Filled by `commit` for singleton commitments, or replaced by the
        // vector-leaf batch root in optimized `batch_commit`.
        codeword_tree: vec![vec![Output::<H>::default()]],
        batch_codeword_tree: Vec::new(),
        batch_root: None,
        bh_evals,
    ...
    V: BasefoldExtParams,
{
    ...
    fn commit(pp: &Self::ProverParam, poly: &Self::Polynomial) -> Result<Self::Commitment, Error> {
        // Single-polynomial commitment keeps the original behavior: encode the
        // polynomial and build its ordinary Merkle tree. Batched commitments
        // below use `commit_without_individual_tree` so that QABase does not
        // build m ordinary trees and then an extra vector-leaf tree.
        let mut comm = commit_without_individual_tree::<F, H>(pp, poly)?;
        comm.codeword_tree = merkelize::<F, H>(&comm.codeword);
        Ok(comm)
    }
        ...
        Self::Polynomial: 'a,
    {
        ...
        // If this is a real batch commitment, also bind the vector-leaf Merkle
        // root at commit time. `batch_open` will later use this root to
        // authenticate one shared path per query across all committed codewords.
        if comms.len() > 1 {
            let batch_root = comms[0]
                .batch_root
                .as_ref()
                .expect("batch_commit must set batch_root for batched commitments");
            transcript.write_commitment(batch_root).unwrap();
        }
    ...
    fn batch_commit<'a>(
        ...
        // Optimized batched commitment path.
        //
        // Each polynomial is still BaseFold-encoded independently, but we do
        // not build its ordinary Merkle tree. Instead, all codewords are
        // authenticated by one vector-leaf Merkle tree. This is the path used
        // by QABase's auxiliary commitments and removes the previous prover
        // overhead of constructing m ordinary trees plus one batch tree.
        let mut comms: Vec<Self::Commitment> = polys_vec
            .par_iter()
            .map(|poly| commit_without_individual_tree::<F, H>(pp, poly))
            .collect::<Result<Vec<_>, _>>()?;

        let comm_refs = comms.iter().collect_vec();
        let batch_tree = merkelize_batch_codewords_from_refs::<F, H>(&comm_refs);
        let batch_root = batch_tree[batch_tree.len() - 1][0].clone();

        // Store the large batch tree only once, on the first commitment. For
        // compatibility with the existing PCS trait, each commitment also
        // exposes a root through `codeword_tree`; for batched commitments this
        // root is the vector-leaf batch root, not an individual codeword root.
        comms[0].batch_codeword_tree = batch_tree;
        for comm in comms.iter_mut() {
            comm.batch_root = Some(batch_root.clone());
            comm.codeword_tree = vec![vec![batch_root.clone()]];
        }
    ...
    fn batch_open<'a>(
        ...
        // Re-absorb every batch-root group into the opening transcript.  A
        // QABase opening typically contains two commitment groups: the online
        // auxiliary witness commitments and the preprocessed public E_i
        // commitments.  Each group has its own vector-leaf Merkle root, so the
        // opening proof will authenticate one shared path per group and query.
        let batch_groups = collect_batch_groups::<F, H>(&comms);
        for (_, root) in batch_groups.iter() {
        ...
        // Queries to the original committed codewords for the batched opening.
        //
        // Previous implementation authenticated each `(comm, query)` pair with
        // an independent Merkle path. We instead authenticate all committed
        // codewords at the same query index using one vector-leaf batch Merkle
        // path. We still send the two field elements per committed codeword,
        // because the verifier must locally compute the random linear
        // combination used in the batched claim.
        let mut individual_queries: Vec<Vec<(F, F)>> = Vec::with_capacity(queries_usize.len());

        let batch_trees: Vec<Cow<'_, Vec<Vec<Output<H>>>>> = batch_groups
            .iter()
            .map(|(indices, _)| {
                if indices.len() == 1 {
                    Cow::Borrowed(&comms[indices[0]].codeword_tree)
                } else {
                    let first = indices[0];
                    if !comms[first].batch_codeword_tree.is_empty() {
                        Cow::Borrowed(&comms[first].batch_codeword_tree)
                    } else {
                        // Compatibility fallback for tests.  In the intended
                        // protocol path, this tree is built during
                        // batch_commit and its root is already in the transcript.
                        let group_comms = indices.iter().map(|&i| comms[i]).collect_vec();
                        Cow::Owned(merkelize_batch_codewords_from_refs::<F, H>(&group_comms))
                    }
                }
            })
            .collect_vec();

        let mut batch_paths: Vec<Vec<Vec<(Output<H>, Output<H>)>>> =
            Vec::with_capacity(queries_usize.len());

        for query in &queries_usize {
            let mut comm_queries = Vec::with_capacity(comms.len());
            for c in &comms {
                comm_queries.push(query_codeword_pair::<F>(query, &c.codeword.poly));
            }
            individual_queries.push(comm_queries);

            let paths_for_query = batch_trees
                .iter()
                .map(|tree| get_merkle_path::<H, F>(tree.as_ref(), *query, true))
                .collect_vec();
            batch_paths.push(paths_for_query);
        }
    ...
    fn read_commitments(
        ...
        // `batch_commit_and_write` writes one additional vector-leaf batch root
        // when more than one BaseFold polynomial is committed together.
        if num_polys > 1 {
            let batch_root = transcript.read_commitment().unwrap();
            for comm in comms.iter_mut() {
                comm.batch_root = Some(batch_root.clone());
            }
        }
    ...
    fn batch_verify<'a>(
        ...
        let opening_roots = transcript.read_commitments(comms.len()).unwrap();
        for (root, comm) in opening_roots.iter().zip(comms.iter()) {
            let expected_root = &comm.codeword_tree[comm.codeword_tree.len() - 1][0];
            assert_eq!(root, expected_root, "BaseFold batch opening root mismatch");
        }

        let batch_groups = collect_batch_groups::<F, H>(&comms);
        for (_, expected_root) in batch_groups.iter() {
            if let Some(expected_root) = expected_root {
                let batch_root = transcript.read_commitment().unwrap();
                assert_eq!(&batch_root, expected_root, "BaseFold batch root mismatch");
            }
        }
        ...
        // Read one vector-leaf Merkle path per commitment group and verifier
        // query. Each path authenticates all codeword sibling-pairs belonging
        // to that group at the queried position.
        let mut batch_paths = Vec::with_capacity(vp.num_verifier_queries);
        for _ in 0..vp.num_verifier_queries {
            let mut paths_for_query = Vec::with_capacity(batch_groups.len());
            for _ in 0..batch_groups.len() {
                ...
                paths_for_query.push(chunked_path);
            }
            batch_paths.push(paths_for_query);
        }
        ...
        for (i, query) in queries.iter().enumerate() {
            ...
            for j in 0..scalars.len() {
                let poly_idx = evals[j].poly();
                lc0 += scalars[j] * ind_queries[i][poly_idx][0];
                lc1 += scalars[j] * ind_queries[i][poly_idx][1];
            }
        ...
        for vq in 0..vp.num_verifier_queries {
            for (group_idx, (indices, group_root)) in batch_groups.iter().enumerate() {
                let mut path = batch_paths[vq][group_idx].clone();

                let root_pair = path
                    .pop()
                    .expect("batch Merkle path should contain the committed root pair");
                let path_root = root_pair
                    .last()
...
///
/// Commitments that share the same `batch_root` were produced by the same
/// `batch_commit` call and can share one vector-leaf Merkle authentication path
/// per BaseFold query. Commitments without a `batch_root` are treated as
/// singleton groups and use their ordinary Merkle root.
fn collect_batch_groups<F: PrimeField, H: Hash>(
    comms: &[&BasefoldCommitment<F, H>],
) -> Vec<(Vec<usize>, Option<Output<H>>)> {
    let mut groups: Vec<(Vec<usize>, Option<Output<H>>)> = Vec::new();

    for (idx, comm) in comms.iter().enumerate() {
        if let Some(root) = &comm.batch_root {
            if let Some((indices, _)) = groups
                .iter_mut()
                .find(|(_, existing)| existing.as_ref() == Some(root))
            {
                indices.push(idx);
            } else {
                groups.push((vec![idx], Some(root.clone())));
            }
        } else {
            groups.push((vec![idx], None));
        }
    }
...
/// Build a vector-leaf Merkle tree for a batch of BaseFold commitments.
///
/// The i-th leaf hashes all sibling-pair values at codeword positions 2i and
/// 2i+1 across the whole batch:
///   H(C_0[2i], C_0[2i+1], ..., C_{m-1}[2i], C_{m-1}[2i+1]).
///
/// This is used by the optimized `batch_open` to replace `m` independent
/// Merkle paths per verifier query by a single shared Merkle path.
fn merkelize_batch_codewords_from_refs<F: PrimeField, H: Hash>(
    comms: &[&BasefoldCommitment<F, H>],
) -> Vec<Vec<Output<H>>> {
    assert!(!comms.is_empty(), "empty BaseFold batch commitment");

    let codeword_len = comms[0].codeword.poly.len();
    assert!(codeword_len.is_power_of_two());
    assert!(codeword_len >= 2);

    for comm in comms {
        assert_eq!(
            comm.codeword.poly.len(),
            codeword_len,
            "all BaseFold codewords in a batch must have the same length"
        );
    }

    let log_v = log2_strict(codeword_len);
    let mut tree = Vec::with_capacity(log_v);

    let mut hashes = vec![Output::<H>::default(); codeword_len >> 1];
    hashes.par_iter_mut().enumerate().for_each(|(i, hash)| {
        ...
        for comm in comms {
            hasher.update_field_element(&comm.codeword.poly[left]);
            hasher.update_field_element(&comm.codeword.poly[right]);
        }
...
fn query_codeword_pair<F: PrimeField>(query: &usize, codeword: &Vec<F>) -> (F, F) {
    let mut p0 = *query;
    let temp = p0;
    let mut p1 = p0 ^ 1;
    if p1 < p0 {
...
fn authenticate_batch_merkle_path<H: Hash, F: PrimeField>(
    ...
    for (left, right) in leaves {
        hasher.update_field_element(left);
        hasher.update_field_element(right);
    }
...
fn verifier_query_phase<F: PrimeField, H: Hash>(
    ...
    println!("VERIFIER QUERY PHASE");
    println!("CHALLENGES LENGTH {:?}", query_challenges.len());

    // Multi-query support.
    //
    // The prover writes one query transcript for each verifier query.
    // Therefore the verifier must check all query transcripts, not only one.
    assert_eq!(
        queries.len(),
        query_challenges.len(),
        "number of query transcripts must match number of query challenges"
    );

    assert_eq!(
        query_merkle_paths.len(),
        query_challenges.len(),
        "number of Merkle query paths must match number of query challenges"
    );

    assert_eq!(
        roots.len(),
        num_rounds,
        "BaseFold should contain one committed root per folding round"
    );

    assert_eq!(
        fold_challenges.len(),
        num_rounds,
        "BaseFold should contain one folding challenge per folding round"
    );

    let n = 1usize << (num_vars + log_rate);
    ...
    // Reconstruct the same randomness used to generate the BaseFold folding domain.
    ...
    queries_usize
        ...
        .for_each(|(qi, query_index)| {
            assert!(
                queries[qi].len() >= num_rounds + 1,
                "each query transcript must contain num_rounds + 1 query pairs"
            );

            assert!(
                query_merkle_paths[qi].len() >= num_rounds,
                "each query must contain one Merkle path per folding round"
            );
            ...
            for i in 0..num_rounds {
                ...
                assert_eq!(
                    folded_query,
                    cur_queries[i + 1][(cur_index >> 1) % 2],
                    "BaseFold query folding consistency failed"
                );
    ...
    // Sumcheck consistency checks.
    assert_eq!(
        eval,
        &degree_2_zero_plus_one(&sum_check_oracles[0]),
        "BaseFold first sumcheck claim mismatch"
    );

    for i in 0..fold_challenges.len().saturating_sub(1) {
        assert_eq!(
            degree_2_eval(&sum_check_oracles[i], fold_challenges[i]),
            degree_2_zero_plus_one(&sum_check_oracles[i + 1]),
            "BaseFold sumcheck round consistency failed"
        );
    }
